...
function inspect(root, targetFile, options) {
    ...
    const dockerArchivePath = (options && options.dockerArchivePath) || undefined;
    return dockerFile
        .readDockerfileAndAnalyse(targetFile)
        .then((dockerfileAnalysis) => {
        return Promise.all([
            getRuntime(dockerOptions),
            getDependencies(targetImage, dockerfileAnalysis, dockerOptions, dockerArchivePath),
        ]).then((res) => {
...
function getDependencies(targetImage, dockerfileAnalysis, options, dockerArchivePath) {
    let result;
    return analyzer
