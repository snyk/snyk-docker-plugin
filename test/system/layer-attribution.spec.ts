...
// 2. The label's value is a real diffID from the image's `rootFs`, and the
//    diffID -> `createdBy` join (via the OS scan result's `rootFs` /
//    `history` facts) resolves to the install instruction.
// 3. Without `--layer-attribution`, no dep-graph node carries the label.
//
// NOTE: the rootFs/history *duplication onto app scan results* part of the
// design is covered by unit tests in `test/lib/response-builder.spec.ts`,
// not here — this fixture is OS-only (apk) and produces no app scan result,
// so it cannot exercise that path.
//
...
describe("layer attribution wire format on a real image", () => {
  ...
  describe("with --layer-attribution enabled", () => {
    ...
    beforeAll(async () => {
      ...
      osDepGraph = depGraphFact.data;
      labelledNodes = findLabelledNodes(osDepGraph);
    ...
    it("omits htop entirely — it was installed and later purged, so it is not even a dep-graph package", () => {
      // htop is installed by `apk add jq htop` and removed by the
      // following RUN, so it is no longer on disk in the final image.
      // Assert it is absent from the dep graph altogether (not merely
      // unlabelled) — that's what distinguishes "purged" from "present
      // but missed by attribution".
      const htop = osDepGraph
        .getPkgs()
        .find((p) => /(?:^|\/)htop$/.test(p.name));
