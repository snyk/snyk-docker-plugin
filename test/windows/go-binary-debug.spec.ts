...
import { getGoModulesContentAction, goModulesToScannedProjects } from "../../lib/go-parser";
...
describe("Go Binary Detection Debug (Windows)", () => {
  ...
  it("should debug full scan process and output goModulesToScannedProjects results", async () => {
    ...
    console.log(`🔍   scanResults.length: ${pluginResult.scanResults?.length || 0}`);
    ...
    if (pluginResult.scanResults) {
      pluginResult.scanResults.forEach((result, index) => {
        ...
        console.log(`🔍     identity.targetFile: ${result.identity.targetFile || 'N/A'}`);
        console.log(`🔍     facts.length: ${result.facts?.length || 0}`);
        
        const depGraphFact = result.facts?.find(f => f.type === 'depGraph');
        if (depGraphFact) {
          console.log(`🔍     depGraph.rootPkg.name: ${depGraphFact.data?.rootPkg?.name}`);
          console.log(`🔍     depGraph.deps.length: ${depGraphFact.data?.getDepPkgs?.()?.length || 0}`);
        }
      });
    }
  ...
  it("should debug direct Go binary extraction and processing", async () => {
    ...
    try {
      ...
      Object.entries(extractionResult.extractedLayers).forEach(([filePath, layerContent]) => {
      ...
      if (Object.keys(goModulesContent).length > 0) {
        ...
        const scannedProjects = await goModulesToScannedProjects(goModulesContent);
        ...
        scannedProjects.forEach((project, index) => {
          ...
          console.log(`🔍   identity.targetFile: ${project.identity.targetFile}`);
          console.log(`🔍   facts.length: ${project.facts?.length || 0}`);
          
          const depGraphFact = project.facts?.find(f => f.type === 'depGraph');
          if (depGraphFact) {
            console.log(`🔍   depGraph.rootPkg.name: ${depGraphFact.data?.rootPkg?.name}`);
            console.log(`🔍   depGraph.deps.length: ${depGraphFact.data?.getDepPkgs?.()?.length || 0}`);
          }
        ...
        if (scannedProjects.length === 0) {
          console.log("🔍 ❌ WARNING: No Go projects found in scannedProjects!");
        } else {
          console.log(`🔍 ✅ SUCCESS: Found ${scannedProjects.length} Go project(s)`);
        }
      } else {
        console.log("🔍 ❌ WARNING: No Go modules content found in extracted layers!");
