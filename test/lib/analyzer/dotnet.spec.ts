...
describe("dotnet deps.json analyzer", () => {
  ...
  describe("framework package filtering", () => {
    it("should exclude runtime pack packages", async () => {
      const content = loadFixture("WithFramework.deps.json");
      const filePathToContent = {
        "/app/WithFramework.deps.json": content,
      };
      ...
      const newtonsoft = pkgs.find((p) => p.name === "Newtonsoft.Json");
      expect(newtonsoft).toBeDefined();
      expect(newtonsoft!.version).toBe("13.0.3");

      const runtimePack = pkgs.find((p) =>
        p.name.includes("Microsoft.NETCore.App.Runtime"),
      );
      expect(runtimePack).toBeUndefined();
    });

    it("should exclude packages with libraries type platform", async () => {
      const depsJson = JSON.parse(loadFixture("WithFramework.deps.json"));
      depsJson.targets[".NETCoreApp,Version=v8.0"][
        "Contoso.FuturePlatform/9.0.0"
      ] = {};
      depsJson.libraries["Contoso.FuturePlatform/9.0.0"] = {
        type: "platform",
        serviceable: true,
        sha512: "",
        path: "contoso.futureplatform/9.0.0",
        hashPath: "",
      };
      depsJson.targets[".NETCoreApp,Version=v8.0"][
        "WithFramework/1.0.0"
      ].dependencies["Contoso.FuturePlatform"] = "9.0.0";

      const results = await dotnetFilesToScannedProjects({
        "/app/WithFramework.deps.json": JSON.stringify(depsJson),
      });
      const pkgNames = results[0].facts
        .find((f) => f.type === "depGraph")!
        .data.getPkgs()
        .map((p) => p.name);

      expect(pkgNames).not.toContain("Contoso.FuturePlatform");
    });

    it("should exclude packages with Microsoft.NETCore.App prefix", async () => {
      const content = loadFixture("WithFramework.deps.json");
      const filePathToContent = {
        "/app/WithFramework.deps.json": content,
      };
      ...
      const pkgNames = depGraph.getPkgs().map((p) => p.name);

      expect(
        pkgNames.some((n) =>
          n.toLowerCase().startsWith("microsoft.netcore.app"),
        ),
      ).toBe(false);
    });
  });
