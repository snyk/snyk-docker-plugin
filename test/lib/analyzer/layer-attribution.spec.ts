...
describe("computeOsLayerAttribution", () => {
  ...
  it("attributes only the primary (first non-empty) ecosystem on a mixed image", async () => {
    // A real image carries a single OS PM, and the dep graph is built from
    // the first non-empty analysis (`selectPrimaryPackageAnalysis`).
    // Attribution follows the same selection, so on a (synthetic) two-PM
    // image only the first ecosystem is attributed and the second is ignored
    // — matching the dep graph's own choice, by construction.
    const { introducingLayerByPackage, warnings } = await merge(
      [
        makeAnalysis(AnalysisType.Apk, curl),
    ...
    expect(introducingLayerByPackage.get("curl@7.0.0-r0")).toBe("sha256:a");
    expect(introducingLayerByPackage.has("libc6@2.35-0ubuntu3")).toBe(false);
    expect(warnings).toEqual([]);
  });
  ...
  it("ignores ecosystems whose top-level Analysis is empty", async () => {
    ...
    expect(introducingLayerByPackage.size).toBe(1);
    expect(introducingLayerByPackage.get("curl@7.0.0-r0")).toBe("sha256:a");
  });
  ...
  it("records a failure attributing the primary ecosystem as a warning instead of throwing", async () => {
    // A diffIDs/orderedLayers length mismatch makes the per-PM call throw;
    // the orchestrator must collect the error and still return.
    const { introducingLayerByPackage, warnings } = await merge(
      [makeAnalysis(AnalysisType.Apk, curl)],
      [makeApkLayer(makeApkDb(curl))],
    ...
    expect(warnings[0]).toContain("Apk");
    expect(warnings[0]).toMatch(/orderedLayers \(1\) and diffIDs \(2\)/);
    expect(introducingLayerByPackage.size).toBe(0);
  });

  it("does not attribute a second ecosystem even when its keys would collide", async () => {
    // Both PMs would mint the bare key `curl@...`, but only the primary
    // (first non-empty) ecosystem is attributed, so there is no collision,
    // no warning, and the diffID is unambiguously the primary's.
    const { introducingLayerByPackage, warnings } = await merge(
      [
        makeAnalysis(AnalysisType.Apk, curl),
