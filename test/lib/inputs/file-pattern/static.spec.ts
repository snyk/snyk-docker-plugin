import { generateExtractAction, getMatchingFiles } from "../../../../../lib/inputs/file-pattern/static";
...
describe("file-pattern static", () => {
  describe("generateExtractAction", () => {
    it("should exclude files matching exclusion patterns", async () => {
      ...
      expect(extractAction.filePathMatches("/app/node_modules/package.json")).toBe(false);
      expect(extractAction.filePathMatches("/test/fixtures/config.yaml")).toBe(false);
  ...
  describe("getMatchingFiles", () => {
    it("should throw an error when extracted data is not a string", () => {
      ...
      expect(() => getMatchingFiles(extractedLayers)).toThrow("expected a string");
    });
    ...
    it("should handle multiple files with base64 content", () => {
      const extractedLayers: ExtractedLayers = {
        ...
        "/app/settings.json": {
          "find-files-by-pattern": Buffer.from('{"setting": true}').toString("base64")
        }
      };
      ...
      expect(configFile?.contents).toBe(Buffer.from("key: value").toString("base64"));
      
      const settingsFile = result.find(f => f.name === "settings.json");
      expect(settingsFile?.path).toBe("/app");
      expect(settingsFile?.contents).toBe(Buffer.from('{"setting": true}').toString("base64"));
    });
  });
});