...
test("getFileProduct", async (t) => {
  ...
  t.test("file product with callback", async (t) => {
    const content = await docker.getFileProduct("/some/file", "md5", md5);
  ...
  const examples = {
    "ubuntu:10.04": {
      ...
      expected: {
        ...
        md5: ["/etc/debian_version", "/etc/lsb-release"],
      },
    },
  };
  ...
  for (const targetImage of Object.keys(examples)) {
    await t.test(targetImage, async (t) => {
      ...
      for (const extractActionName of Object.keys(example.expected)) {
        const contents = await docker.getFileProducts(extractActionName);
        // same number of items
        t.equal(
          Object.keys(contents).length,
          example.expected[extractActionName].length,
        );
        // contains all expected items
        for (const filename of example.expected[extractActionName]) {
          t.ok(
            Object.keys(contents).includes(filename),
            `${filename} ${extractActionName}`,
          );
        }
      }
    });
