...
`;
  ...
  describe(" handles malformed input safely", () => {
    describe("input size limits", () => {
      ...
      it("returns null when JAVA_VERSION value is excessively long", () => {
        const longVersion = "1".repeat(10000);
        const content = `JAVA_VERSION="${longVersion}"`;
        const result = parseJavaRuntimeRelease(content);
        expect(result).toBeNull();
      });

      it("returns null for extremely long single line", () => {
        const longLine = "JAVA_VERSION=" + "1.0.".repeat(100000) + "1";
        const result = parseJavaRuntimeRelease(longLine);
        expect(result).toBeNull();
      });
    });
    ...
    describe("edge cases with special content", () => {
      ...
      it("handles content with mixed null bytes and valid data", () => {
        ...
        const result = parseJavaRuntimeRelease(content);
        expect(() => parseJavaRuntimeRelease(content)).not.toThrow();
      });
