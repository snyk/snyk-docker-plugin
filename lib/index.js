...
function inspect(root, targetFile, options) {
  if (options && options['new-analyzer']) {
    console.log('using new analyzer');
  }
  ...
  return fetchSnykDockerAnalyzer()
    .then(function (analyzerBinaryPath) {
      return Promise.all([
        getRuntime(),
        getDependencies(analyzerBinaryPath, targetImage, options || {}),
      ])
...
function getDependencies(analyzerBinaryPath, targetImage, options) {
  var useNewAnalyzer = !!options['new-analyzer'];
  var analyzerPromise = useNewAnalyzer
    ? analyzer.analyze(targetImage)
    : subProcess.execute(analyzerBinaryPath, buildArgs(targetImage));

  var result;

  return analyzerPromise
    .then(function (output) {
      result = parseAnalysisResults(output, useNewAnalyzer);
      return buildTree(
...
function parseAnalysisResults(analysisOut, newAnalyzer) {
  var analysisJson = newAnalyzer ? analysisOut : JSON.parse(analysisOut);
