...
function getTargetOSFromScanResult(scanResult: ScanResult, testResult: TestResult): string | null {
  // First check for imageOsReleasePrettyName fact
  const osReleaseFact = scanResult.facts?.find(fact => fact.type === 'imageOsReleasePrettyName');
